...
impl pallet_ghost_consensus::Config for Runtime {
	...
	type DoubleSignSlashPercentage = ConstU8<50>; // 50% slash for double signing
	type InvalidBlockSlashPercentage = ConstU8<25>; // 25% slash for invalid blocks
	type DowntimeSlashPercentage = ConstU8<10>; // 10% slash for downtime
	type PalletId = frame_support::PalletId(*b"py/ghost");
}
