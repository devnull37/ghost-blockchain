...
pub fn calculate_difficulty_adjustment<T: Config>(
	...
	let mut adjustment_factor = if actual_block_time < target_block_time {
	...
	// If entropy is low (centralization), we increase difficulty significantly.
	// We'll use 4,000,000 (log2(16)) as a "healthy decentralization" threshold.
	let entropy_threshold = 4_000_000;
	if entropy < entropy_threshold {
		// Increase difficulty multiplier if entropy is low.
		// multiplier = 1 + (threshold - entropy) / threshold
		// We scale it by 100 for our adjustment_factor.
		let entropy_penalty = ((entropy_threshold - entropy) * 100) / entropy_threshold;
		adjustment_factor = adjustment_factor.saturating_add(entropy_penalty);
	}
