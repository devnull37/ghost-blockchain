...
where
    ...
    C::Api: GhostConsensusRuntimeApi<AccountId, Balance>,
    AccountId: Codec,
    Balance: Codec,
{
    ...
    fn get_validator_stake(
        ...
        api.get_validator_stake(at, validator)
            .map_err(runtime_error_into_rpc_error)
    }
    ...
    fn get_slashing_records(&self, at: Option<Block::Hash>) -> RpcResult<u32> {
        ...
        api.get_slashing_records_count(at)
            .map_err(runtime_error_into_rpc_error)
    }
}
